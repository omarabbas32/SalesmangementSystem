    ...
    try {
        ...
        if (contentType && contentType.includes('application/json')) {
            ...
            if (textResponse.includes('<html') || textResponse.includes('<!DOCTYPE')) {
                throw new Error('تم استلام صفحة HTML بدلاً من JSON - تحقق من أن الخادم يعمل بشكل صحيح');
            }
...
const todayExpenses = dashboardData?.data?.stats?.todayExpenses || {};
        ...
        document.getElementById('today-sales').textContent = `${todaySales.total || 0} جنيه مصري`;
        document.getElementById('today-expenses').textContent = `${todayExpenses.total || 0} جنيه مصري`;
        document.getElementById('today-profit').textContent = `${(todaySales.total || 0) - (todayExpenses.total || 0)} جنيه مصري`;
        ...
        if (lowStockData?.data?.products && lowStockData.data.products.length > 0) {
            ...
            lowStockList.innerHTML = '<p style="text-align: center; color: #6c757d;">لا توجد منتجات منخفضة المخزون</p>';
        }
    } catch (error) {
...
function renderProducts() {
    ...
    if (currentProducts.length === 0) {
        productsGrid.innerHTML = '<p style="text-align: center; color: #6c757d; grid-column: 1 / -1;">لا توجد منتجات</p>';
        return;
    }
    ...
    currentProducts.forEach(product => {
        ...
        const stockPercentage = Math.min((product.current_stock_grams / 5000) * 100, 100);
        const isLowStock = product.current_stock_grams < 1000;
        
        productCard.innerHTML = `
            <div class="product-header">
                <h3 class="product-name">${product.name}</h3>
                <span class="product-price">${product.price_per_kg} جنيه مصري/كيلو</span>
            </div>
            <div class="product-stock">
                <div class="stock-info">
                    <span class="stock-grams">${product.current_stock_grams} جرام</span>
                    <span class="stock-kg">(${Math.round(product.current_stock_grams / 1000 * 1000) / 1000} كيلو)</span>
                </div>
                <div class="stock-bar">
            ...
            <div class="product-actions">
                <button class="btn btn-success btn-sm" onclick="showAddStockModal('${product.id}')">
                    <i class="fas fa-plus"></i> إضافة مخزون
                </button>
                <button class="btn btn-warning btn-sm" onclick="showUpdatePriceModal('${product.id}', ${product.price_per_kg})">
...
function renderSales() {
    ...
    if (currentSales.length === 0) {
        salesList.innerHTML = '<p style="text-align: center; color: #6c757d;">لا توجد مبيعات</p>';
        return;
    }
    ...
    currentSales.forEach(sale => {
        ...
        saleItem.innerHTML = `
            <div class="item-header">
                ...
                <span class="item-amount">${sale.total_amount} جنيه مصري</span>
            </div>
            <div class="item-details">
                ...
                <div class="detail-item">
                    ...
                    <span class="detail-value">${sale.price_per_kg} جنيه مصري</span>
                </div>
                <div class="detail-item">
            ...
            <div class="item-actions">
                <button class="btn btn-primary btn-sm" onclick="printInvoice(${sale.id})">
                    <i class="fas fa-print"></i> طباعة فاتورة
                </button>
                <button class="btn btn-danger btn-sm" onclick="cancelSale(${sale.id})">
...
function renderExpenses() {
    ...
    if (currentExpenses.length === 0) {
        expensesList.innerHTML = '<p style="text-align: center; color: #6c757d;">لا توجد مصروفات</p>';
        return;
    }
    ...
    currentExpenses.forEach(expense => {
        ...
        const expenseDate = new Date(expense.expense_date).toLocaleDateString('ar-EG');
        const expenseTime = new Date(expense.expense_date).toLocaleTimeString('ar-EG');
...
function renderInventory(inventory, stats) {
    ...
    if (inventory.length === 0) {
        inventoryList.innerHTML = '<p style="text-align: center; color: #6c757d;">لا توجد منتجات في المخزون</p>';
        return;
    }
    ...
    inventory.forEach(item => {
        ...
        const stockPercentage = Math.min((item.current_stock_grams / 5000) * 100, 100);
        ...
        inventoryItem.innerHTML = `
            ...
            <div class="inventory-details">
                <div class="detail-row">
                    ...
                    <span>${item.current_stock_grams} جرام (${Math.round(item.current_stock_grams / 1000 * 1000) / 1000} كيلو)</span>
                </div>
                <div class="detail-row">
                ...
                <div class="detail-row">
                    ...
                    <span>${Math.round((item.current_stock_grams / 1000) * item.price_per_kg * 100) / 100} جنيه مصري</span>
                </div>
            </div>
            <div class="stock-bar">
                <div class="stock-fill ${isLowStock ? 'low' : ''}" style="width: ${stockPercentage}%"></div>
...
function renderDailyReport(report) {
    ...
    reportResults.innerHTML = `
        <div class="report-summary">
            <h3>تقرير يومي - ${new Date(report.date).toLocaleDateString('ar-EG')}</h3>
            <div class="summary-cards">
                <div class="summary-card">
                    <h4>إجمالي المبيعات</h4>
                    <span class="amount sales">${report.sales.total} جنيه مصري</span>
                </div>
                <div class="summary-card">
                    <h4>إجمالي المصروفات</h4>
                    <span class="amount expenses">${report.expenses.total} جنيه مصري</span>
                </div>
                <div class="summary-card">
                    <h4>صافي الربح</h4>
                    <span class="amount profit">${report.summary.netProfit} جنيه مصري</span>
                </div>
            </div>
        </div>
        ...
        <div class="report-details">
            ...
            <div class="expenses-details">
                <h4>تفاصيل المصروفات (${report.expenses.items.length} مصروف)</h4>
                <div class="details-list">
...
document.getElementById('add-product-form').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        const productData = {
            ...
            initialStockGrams: parseFloat(document.getElementById('product-stock').value) || 0
        };
...
document.getElementById('sale-product').addEventListener('change', (e) => {
    ...
    if (selectedOption && selectedOption.dataset.price) {
        document.getElementById('sale-price-per-kg').value = selectedOption.dataset.price;
        calculateSaleTotal();
    }
});

document.getElementById('sale-weight').addEventListener('input', calculateSaleTotal);

function calculateSaleTotal() {
    const pricePerKg = parseFloat(document.getElementById('sale-price-per-kg').value);
    const weightGrams = parseFloat(document.getElementById('sale-weight').value);
...
async function printInvoice(saleId) {
    try {
        const response = await apiCall(`/sales/${saleId}/invoice`);
            ...
            <html>
                <head>
                    ...
                    <style>
                        ...
                        .invoice-total { font-size: 18px; font-weight: bold; text-align: center; }
                    </style>
                </head>
                <body>
                    <div class="invoice-header">
                        ...
                        <p>التاريخ: ${invoice.formattedDate}</p>
                    </div>
                    <div class="invoice-details">
                        <p><strong>المنتج:</strong> ${invoice.product_name}</p>
                        <p><strong>الوزن:</strong> ${invoice.weightKg} كيلو</p>
                        <p><strong>سعر الكيلو:</strong> ${invoice.price_per_kg} جنيه مصري</p>
                    </div>
                    <div class="invoice-total">
                        <p>المبلغ الإجمالي: ${invoice.total_amount} جنيه مصري</p>
                    </div>
                </body>
            </html>
        `);
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        if (testResponse.ok) {
            ...
            showToast('تحذير: مشكلة في الاتصال بالخادم - ' + testResponse.status, 'warning');
        }
    } catch (error) {
